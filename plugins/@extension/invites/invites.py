...
class Invites(commands.Cog):
    ...
    async def _get_or_create_webhook(
        ...
        if bot_me and not channel.permissions_for(bot_me).manage_webhooks:
            return None
    ...
    @commands.group(
        name="invite", aliases=["invites", "inv"], invoke_without_command=True
    )
    ...
    @invite_.group(name="config", invoke_without_command=True)
    ...
    @invite_config.group(name="set", invoke_without_command=True)
    @checks.has_permissions(PermissionLevel.ADMINISTRATOR)
    async def invite_config_set(self, ctx: commands.Context):
        """
        Set the configurations.
        """
        await ctx.send_help(ctx.command)

    @invite_config_set.command(name="channel")
    @checks.has_permissions(PermissionLevel.ADMINISTRATOR)
    async def invite_config_set_channel(
    ...
    @invite_config_set.command(name="enable")
    @checks.has_permissions(PermissionLevel.ADMINISTRATOR)
    async def inv_config_set_enable(self, ctx: commands.Context, mode: bool):
        """
    ...
    @invite_config.command(name="get")
    @checks.has_permissions(PermissionLevel.ADMINISTRATOR)
    async def invite_config_get(self, ctx: commands.Context, *, key: str.lower = None):
        """
    ...
    @invite_config.command(name="reset")
    @checks.has_permissions(PermissionLevel.ADMINISTRATOR)
    async def invite_config_reset(self, ctx: commands.Context):
        """
    ...
    @invite_.command(name="refresh")
    @checks.has_permissions(PermissionLevel.OWNER)
    async def invite_refresh(self, ctx: commands.Context):
        """
    ...
    @invite_.command(name="info")
    @checks.has_permissions(PermissionLevel.MODERATOR)
    async def invite_info(self, ctx: commands.Context, invite: discord.Invite):
        """
        Get an info of a specific invite.
        """
        embed = discord.Embed(color=self.bot.main_color, title="__Invite info__")
        embed.set_thumbnail(url=str(invite.guild.icon.url))
        embed.description = (
            f"**Server:**\n{invite.guild}\n" f"**Invite link:**\n{invite.url}\n"
        )
    ...
    @invite_.command(name="delete", aliases=["revoke"])
    @checks.has_permissions(PermissionLevel.ADMINISTRATOR)
    async def invite_delete(self, ctx: commands.Context, invite: discord.Invite):
        """
        Delete an invite.
