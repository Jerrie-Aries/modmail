...
if TYPE_CHECKING:
    from .types_ext import ArgsParserRawData, DefaultConfigRaw, ReactRoleConfigRaw
...
YES_EMOJI = "✅"
NO_EMOJI = "❌"  # as of now this emoji is no longer used
...
class RoleManager(commands.Cog, name="Role Manager"):
    ...
    def lookup(ctx: commands.Context, args: ArgsParserRawData):
        ...
        if args["not-discrim"]:
            ...
            for user in matched:
                if not any(
                    [disc == int(user.discriminator) for disc in args["not-discrim"]]
                ):
                    matched_here.append(user)
            passed.append(matched_here)
