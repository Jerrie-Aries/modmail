...
from cogs.resources.emojis import AnimatedEmoji
...
def parse_delete_message_days(
    args: str,
) -> Tuple[str, Union[Literal[0, 1, 2, 3, 4, 5, 6, 7], int]]:
    """
...
class BanEntryNotFound(commands.BadArgument):
    def __init__(self):
        super().__init__("This member has not been banned before.")


def can_mute():
    """
    Checks if the `mute` command can be invoked.

    This will check whether:
        - The bot has `MANAGE_ROLES` permission.
        - "Muted" role is set.
        - Bot's top role is higher than "Muted" role.

    This check returns `False` together with attr `fail_msg` if it doesn't meet one of the checks above.
    """

    async def predicate(ctx: commands.Context):
        if not ctx.me.guild_permissions.manage_roles:
            predicate.fail_msg = "Need `MANAGE_ROLES` permission."
            return False

        # This will only be used within this cog.
        cog: Optional[ModerationT] = ctx.cog
        config = cog.guild_config(str(ctx.guild.id))
        muted_role = config.muted_role
        if muted_role is None:
            raise NoMuteRole

        if (
            muted_role.is_premium_subscriber()
            or muted_role.is_bot_managed()
            or muted_role.is_integration()
        ):
            predicate.fail_msg = (
                "The mute role that has been set cannot be manually assigned to members. "
                "Please set another role."
            )
            return False

        predicate.fail_msg = "I do not have enough permissions to mute/unmute. Please fix the role hierarchy."
        return ctx.me.top_role > muted_role
...
def can_execute_action(
    ...
    return (
        user.id in ctx.bot.bot_owner_ids
...
class ModConfig:
    ...
    # this dict is nested, must use `deepcopy(x)` to generate new dict from this
    default_config = {
        "log_channel": str(int()),
        "logging": False,
        "mute_config": {"muted_role": str(int()), "muted_members": {}},
    }
    ...
    @classmethod
    def populate(
        cls, bot: ModmailBot, db: Collection, guild: discord.Guild, data: Dict[str, Any]
    ) -> "ModConfig":
        """
        Parameters
        -----------
        ...
        self.db = db
        self.guild = guild
        self._cache = data if data else deepcopy(self.default_config)
    ...
    @property
    def mute_config(self) -> Dict[str, Any]:
        return self.get("mute_config")

    @property
    def muted_role(self) -> Optional[discord.Role]:
        role_id = self.mute_config.get("muted_role")
        return self.guild.get_role(int(role_id))

    def is_muted(self, member: discord.Member) -> bool:
        muted_members = self.mute_config.get("muted_members", {})
        if not muted_members:
            return False
        if str(member.id) not in muted_members:
            return False
        return True
    ...
    def remove(self, key: str) -> Any:
        ...
        self._cache[key] = deepcopy(self.default_config[key])
...
class Moderation(commands.Cog):
    ...
    **NOTE:** You will need the `MODERATOR` permission level in order to run any of these commands.
    ...
    def __init__(self, bot: ModmailBot):
        ...
        self.db: Collection = MISSING  # implemented in `initialize()`
    ...
    async def cog_command_error(
        ...
        handled = False
        if isinstance(error, (BanEntryNotFound, NoMuteRole)):
            handled = True
            ...
            for check in ctx.command.checks:
                ...
                if not checked and hasattr(check, "permission_level"):
                    correct_permission_level = self.bot.command_perm(
                        ctx.command.qualified_name
                    )
                    if correct_permission_level == PermissionLevel.OWNER:
                        continue  # skip if level OWNER
                    embed.description = f"You need permission level `{correct_permission_level.name}` to run this command!"
    ...
    async def initialize(self) -> None:
        ...
        self._handle_automatic_unmutes.start()
    ...
    async def _handle_automatic_unmutes(self) -> None:
        """
        Handle muted members in all guilds the bot is in.

        This task will only run if there is member to unmute, otherwise it will be cancelled.
        And only will run again when someone is muted with `mute` command.
        """
        now_utc = discord.utils.utcnow().timestamp()
        to_cancel = True
        for guild_id in self.config_cache:
            config = self.guild_config(guild_id)
            mute_config = config.mute_config
            muted_members = mute_config.get("muted_members", {})
            if not muted_members:
                continue
            to_cancel = False
            for member_id, member_config in list(muted_members.items()):
                # member_config here is a dict -> {"ends": value, "assigned_role": value}.
                ends = member_config.get("ends", float())

                time_left = ends - now_utc
                if time_left > 60:
                    continue
                role_id = member_config.get("assigned_role", str(int()))
                self.bot.loop.create_task(
                    self.automatic_unmute_member(
                        guild_id, member_id, role_id, time_left
                    )
                )
                muted_members.pop(member_id)  # pop early

        if to_cancel:
            logger.debug("Mute: There are no members to unmute. Stopping the task.")
            self._handle_automatic_unmutes.cancel()

    async def automatic_unmute_member(
        self, guild_id: str, member_id: str, role_id: str, delay: float
    ) -> None:
        if delay <= 1:
            delay = 0  # to prevent negative value for `asyncio.sleep()`
        ...
        assigned_role = guild.get_role(int(role_id))
        if member is None:
            logger.error(
                f"Mute: Unable to find member {member_id} in guild {guild.id}."
            )
        elif assigned_role in member.roles:
            try:
                await member.remove_roles(assigned_role, reason="Mute time ends.")
            except discord.Forbidden:
                logger.error(
                    "Mute: Unable to remove role from member {}, guild {}.".format(
                        member.id, guild.id
                    )
                )
                return
            logger.debug(
                "Mute: Removed Muted role from member {}, guild {}.".format(
                    member.id, guild.id
                )
            )
    ...
    async def logging_enable(self, ctx: commands.Context, mode: bool):
        ...
        embed = discord.Embed(
            description=("Enabled " if mode else "Disabled ")
            + "the logging for moderation commands.",
            color=self.bot.main_color,
        )
        await ctx.send(embed=embed)
        await config.update_db()
    ...
    async def mute(
        ...
        *,
        ...
        The bot must have `Manage Roles` permission and be above the set "Muted" role in the hierarchy.
        """
        ...
        if (duration.dt.timestamp() - duration.now.timestamp()) <= 0:
            raise commands.BadArgument(
                "Unable to parse the `duration` properly. Please try again."
            )

        config = self.guild_config(str(ctx.guild.id))
        muted_role = config.muted_role
        if muted_role is None:
            raise commands.BadArgument("This server does not have a Muted role set up.")
        ...
        # ## Send DM. Currently disabled this.
        # dm_embed = discord.Embed(
        #     title="Mute",
        #     description=(
        #         f"You have been muted for **{human_delta}** from server: {ctx.guild.name}.\n\n"
        #         f"Reason: {reason}"
        #     ),
        ...
        mute_config = config.mute_config
        muted_members = mute_config.get("muted_members", {})
        ends_utc = duration.dt.replace(tzinfo=timezone.utc).timestamp()
        muted_members.update(
            {str(member.id): {"ends": ends_utc, "assigned_role": str(muted_role.id)}}
        )
        ...
        if not self._handle_automatic_unmutes.is_running():
            self._handle_automatic_unmutes.start()
    ...
    @mute.command(usage="<role>")
    @checks.has_permissions(PermissionLevel.ADMINISTRATOR)
    async def setrole(self, ctx: commands.Context, role: discord.Role = None):
        """
        Sets up the `Muted` role.

        `role` if not specified and the mute role hasn't been set, the bot will create a new one with name "Muted".
        If specified, may be a role ID, mention, or name.

        To create a new role, the bot must have `Manage Roles` permission.
        """
        config = self.guild_config(str(ctx.guild.id))
        mute_config = config.mute_config
        if role is None:
            ...
            if muted_role is not None:
                raise commands.BadArgument(
                    f"Muted role was already set to {muted_role.mention}.\n\n"
                    "If you want to change the role, just provide the role ID, name, or mention in the command."
                )
            if not ctx.me.guild_permissions.manage_roles:
            ...
            raise commands.BadArgument(
                "The specified role cannot be manually assigned to members. It is automatically managed by Discord "
                "for Server Boosting or by an Integration (Bot). Please set another role."
            )

        mute_config.update({"muted_role": str(role.id)})

        await ctx.send(
            embed=discord.Embed(
                title="Success",
                description=f"The mute role is now set to {role.mention}.",
                color=self.bot.main_color,
            )
        )
        await config.update_db()
    ...
    async def unmute(
        ...
        **Note(s):**
        - The member will only be unmuted if they were muted by the `mute` command.
        - Only the assigned mute role when the member was muted will be removed.
        """
        config = self.guild_config(str(ctx.guild.id))
        if not config.is_muted(member):
            raise commands.BadArgument(f"{member} is not muted.")

        mute_config = config.mute_config
        muted_members = mute_config.get("muted_members")
        member_config = muted_members[str(member.id)]
        assigned_role = ctx.guild.get_role(int(member_config.get("assigned_role")))

        if reason is None:
            reason = "No reason was provided."

        if assigned_role is not None:
            if not ctx.me.top_role > assigned_role:
                raise commands.BadArgument(
                    "I don't have enough permissions to unmute. Please fix the role hierarchy."
                )

            if assigned_role in member.roles:
                await member.remove_roles(assigned_role, reason=reason)

        muted_members.pop(str(member.id))
        ...
        **Note(s):**
        ...
        if str(member.id) in self.bot.blocked_users:
            ...
            if (
                ...
                and (
                    (
                        datetime.fromisoformat(end_time.group(1)) - after.dt
                    ).total_seconds()
                )
                > 0
            ):
                return
        reason_msg = f"Warn Cooldown: Temporarily blocked due to `warn` command. Required to wait until {after.dt}."
        ...
        *,
        ...
        if not perms.manage_messages or not perms.read_message_history:
            error_embed.description = (
                "Need `MANAGE_MESSAGES` and `READ_MESSAGE_HISTORY` permissions."
            )
        ...
        if limit > max_amount:
            error_embed.description = (
                f"Too many messages to search given ({limit}/100)."
            )
            return await ctx.send(embed=error_embed)
    ...
    async def _remove_user(
        self, ctx: commands.Context, member: discord.Member, search: int = 10
    ):
    ...
    async def _remove_bot(
        self, ctx: commands.Context, prefix: str = None, search: int = 10
    ):
    ...
    async def _remove_reactions(self, ctx: commands.Context, search: int = 10):
        ...
        if search > 100:
            raise commands.BadArgument(
                f"Too many messages to search for ({search}/100)"
            )
    ...
    async def kick(
        ...
        *,
        ...
        try:
            ...
            raise commands.BadArgument(
                "I don't have enough permissions to kick this user."
            )
    ...
    async def ban(
        ...
        **Note(s):**
        ...
        if not isinstance(user, (discord.Member, discord.User)) and isinstance(
            user, int
        ):
            user_id = user
            try:
                ...
                raise commands.BadArgument(
                    '"{}" not found. Invalid user ID.'.format(user_id)
                )
        elif isinstance(user, discord.Member) and not can_execute_action(
            ctx, ctx.author, user
        ):
            raise commands.BadArgument(
                "You cannot do this action on this user due to role hierarchy."
        ...
        try:
            ...
            raise commands.BadArgument(
                "I don't have enough permissions to ban this user."
            )

        banned_emoji = AnimatedEmoji.banned
        embed = discord.Embed(
            title="Ban",
            description=f"{user} has been banned. {banned_emoji}",
            color=self.bot.main_color,
        )
        embed.add_field(name="Reason", value=reason)
        await ctx.send(embed=embed)
    ...
    async def multiban(
        ...
        **Note(s):**
        ...
        if len(members) > 10:
            raise commands.BadArgument(
                f"Too many members to ban given ({len(members)}/10)."
            )
        if reason is None:
            reason = "No reason was provided."
    ...
    async def softban(
        ...
        **Note(s):**
        ...
        await ctx.guild.unban(
            member, reason=f"Moderator - {ctx.author}.\nReason - {reason}"
        )
    ...
    async def unban(
        ...
        *,
        ...
        You can pass either the ID of the banned user or the Name#Discrim combination of the user.
        ...
        await self.send_log(
            ...
            message=f"{user} is now unbanned.",
        )

    @commands.command(aliases=["nick"])
    @checks.has_permissions(PermissionLevel.MODERATOR)
    async def nickname(
        self, ctx: commands.Context, user: discord.Member, *, new_nickname: str = None
    ):
        """
        ...
        **Examples:**
        ...
        for id in parse_args:
            ...
            if not id.startswith("+") and not id.startswith("-"):
                raise commands.BadArgument(
                    f'Missing `+` or `-` symbol for argument "{id}".'
                )
            elif id.startswith("+"):
                id = id.strip("+")
                add = True
